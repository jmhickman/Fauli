/// NetNTLMv2 authentication flow: Negotiate → Challenge → Authenticate.
/// Implements the three-message NTLMSSP exchange per [MS-NLMP].
...
// ---------------------------------------------------------------------------
// Helper: send raw bytes and read response over TCP
// ---------------------------------------------------------------------------
...
let private drainStream (stream : NetworkStream) : byte array =
    // ResizeArray<byte> — amortized O(1) appends for unknown-size reads.
...
// ---------------------------------------------------------------------------
// Build the three-message exchange
// ---------------------------------------------------------------------------
...
let internal parseChallenge (data : byte array) : Result<ChallengeMessage, AuthError> =
    ...
    with ex ->
        Error (mapChallengeParseException ex)
...
// ---------------------------------------------------------------------------
// Full NetNTLMv2 authentication flow
// ---------------------------------------------------------------------------
...
let internal authenticateWithNtlmV2 (client : TcpClient) (username : string) (password : string) (domain : string) (workstation : string option) : Result<NtlmResult, AuthError> =
    ...
    with ex ->
        Error (mapNtlmTransportException ex)
