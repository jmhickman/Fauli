/// NTLMSSP message encoding and decoding per [MS-NLMP].
/// All multi-byte integer fields are little-endian. String fields use UTF-16LE
/// when the UNICODE flag is negotiated; OEM otherwise.
...
// ---------------------------------------------------------------------------
// Helper: write/read little-endian integers
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Helper: encode a payload string field (Len/MaxLen/Offset) into the buffer
// ---------------------------------------------------------------------------
...
let internal encodeNegotiateMessage
    ...
    // Fixed header: 48 bytes (sig 8 + type 4 + flags 4 + domFields 8 + wsFields 8 + version 8)
    let fixedSize = 48
    ...
    // Version (8 bytes, all zero — not used for debugging)
    // bytes 32-39 are already zero
...
let internal encodeAuthenticateMessage (negotiateFlags : uint32) (lmResponse : byte array) (ntResponse : byte array) (domain : string) (username : string) (workstation : string) (encryptedRandomSessionKey : byte array) (mic : byte array) : byte array =
    ...
    // Fixed header: 88 bytes (with Version + MIC)
    let fixedSize = 88
    let totalSize =
