...
// ---------------------------------------------------------------------------
// Domain / principal extraction from usernames
// ---------------------------------------------------------------------------
...
let principalNameFromUserName (userName : UserName) : string =
    let (UserName name) = userName
    match name.Split('@') with
    | [| user; _ |] when not (String.IsNullOrWhiteSpace user) -> user
    | _ ->
        match name.Split('\\') with
        | [| _; user |] when not (String.IsNullOrWhiteSpace user) -> user
        | _ -> name
...
let private domainFromFqdnParts (parts : string array) : DomainName option =
    ...
    | false -> None
    | true ->
        // Take everything after the first label as the domain
